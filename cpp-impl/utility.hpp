...
enum class PuplangError { empty_input, unrecognized, missformed, unknown_casing, invalid_structure, bark_collision};

constexpr auto CASING_MULTIPLIER = static_cast<int>(Casing::Step);
constexpr auto BITS_PER_SOUND = 7;
constexpr uint32_t VALUE_MODULUS = std::pow(2, BITS_PER_SOUND);
...
auto run_combinations(const std::vector<Run>& runs) noexcept {
    return std::accumulate(runs.begin(), runs.end(), 1, [](int acc, const Run& r) {
        return acc * (r.max_extra + 1);
    });
}

auto determine_casing(std::string_view s) -> std::expected<Casing, PuplangError> {
    if (std::ranges::all_of(s, [](unsigned char c) { return std::isupper(c); })) return Casing::Upper;
    if (std::ranges::all_of(s, [](unsigned char c) { return std::islower(c); })) return Casing::Lower;
    if (!s.empty() && std::isupper(static_cast<unsigned char>(s.front()))) return Casing::Title;
    // oh,., bad puppy
...
auto run_length_encode(std::string_view s) -> std::vector<Run> {
    ...
    for (auto chunk : s | std::views::chunk_by(same_char_ci)) {
        char lower = static_cast<char>(std::tolower(static_cast<unsigned char>(chunk.front())));
        runs.push_back({
            .c = lower,
...
auto match_pattern_variation(const std::vector<Run>& in_runs, const WordInfo& base)
    -> std::expected<std::optional<int>, PuplangError> {
    if (base.runs.size() != in_runs.size()) return std::nullopt;
    ...
    for (size_t i = 0; i < base.runs.size(); ++i) {
        ...
        if (in_runs[i].c != base.runs[i].c || extra < 0) return std::nullopt;
...
// Base sound, variation index, casing into a unique linear integer before modulo.
auto calc_sound_value(const WordInfo& base, int var_index, Casing casing) {
    uint32_t base_val = (var_index == 0)
...
auto get_sound_value(std::string_view input, const std::vector<WordInfo>& sounds)
    -> std::expected<uint32_t, PuplangError> {
    if (input.empty()) return std::unexpected(PuplangError::empty_input);

    auto casing = determine_casing(input);
    if (!casing) return std::unexpected(casing.error());
    auto in_runs = run_length_encode(input);

    for (const auto& base : sounds) {
        if (auto var_idx = match_pattern_variation(in_runs, base)) {
            if (!var_idx) return std::unexpected(var_idx.error());
            if (var_idx->has_value()) {
                return calc_sound_value(base, **var_idx, *casing) % VALUE_MODULUS;
            }
        }
    }
...
auto parse_pattern(std::string_view pattern) -> std::vector<Run> {
    ...
    auto update_run = [&](char raw_c) {
        if (auto c = static_cast<char>(std::tolower(static_cast<unsigned char>(raw_c)));
    ...
    for (size_t i = 0; i < pattern.size();) {
        if (pattern[i] == '[') {
            if (auto close = pattern.find(']', i); close != std::string_view::npos && !runs.empty()) {
                auto digits = pattern.substr(i + 1, close - i - 1);
                std::from_chars(digits.data(), digits.data() + digits.size(), runs.back().max_extra);
                i = close + 1;
                continue;
            }
        }
...
struct Settings {
    ...
    static auto load(std::string_view path) -> Settings {
        ...
        while (std::getline(file, line_buf)) {
            ...
            if (line == "[sounds]") { in_sounds = true; continue; }
            if (line.starts_with('[') && line.ends_with(']')) { in_sounds = false; continue; }
            ...
            if (in_sounds) {
                ...
                if (k == "header") cfg.header = v;
                else if (k == "footer") cfg.footer = v;
                else if (k == "prefix_1") cfg.prefixes[1] = v;
                else if (k == "prefix_2") cfg.prefixes[2] = v;
                else if (k == "prefix_3") cfg.prefixes[3] = v;
                else if (k == "prefix_4") cfg.prefixes[4] = v;
            }
        }
        ...
        for (size_t i = 0; i < patterns.size(); ++i) {
            ...
            cfg.sounds.push_back({.runs = runs, .base_index = static_cast<int>(i), .var_offset = offset});
