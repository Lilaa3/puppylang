...
// Deconstructs a formatted token into punctuation, mode prefix, and the core sound
inline auto parse_encoded_word(std::string_view token, const Settings& cfg) -> ParsedWord {
...
inline auto decode(std::string_view encoded_text, const Settings& cfg) -> std::expected<std::string, PuplangError> {
    auto tokens = encoded_text 
            | std::views::split(' ')
    ...
    if (tokens.size() < 2 || tokens.front() != cfg.header || tokens.back() != cfg.footer) {
    ...
    for (size_t i = 1; i + 1 < tokens.size(); ++i) {
        auto [lead, sound, trail, mode_change] = parse_encoded_word(tokens[i], cfg);
        result.append(lead);

        if (mode_change != 0) current_mode = mode_change;

        auto sound_val = get_sound_value(sound, cfg.sounds);
        if (!sound_val) return std::unexpected(sound_val.error());

        current_cp = (current_cp << BITS_PER_SOUND) | (static_cast<uint32_t>(*sound_val) & SOUND_MASK);
        accumulated_chunks++;

        // Once the required number of chunks for the current mode are buffered, emit the codepoint
